/* ============================
   FILTRIRANJE I ANIMACIJE TABELA
=============================== */

// Filtriranje jela po nazivu
function filterTable() {
    let search = document.getElementById("search").value.toLowerCase();
    let rows = document.querySelectorAll("#jelaTable tbody tr");

    rows.forEach(row => {
        let naziv = row.children[1].textContent.toLowerCase();
        row.style.display = naziv.includes(search) ? "" : "none";
    });
}

// Animacija pojavljivanja redova tabele
document.addEventListener("DOMContentLoaded", () => {
    document.querySelectorAll("#jelaTable tbody tr").forEach((tr, i) => {
        tr.style.opacity = 0;
        setTimeout(() => {
...
// Fokus efekat na input polja
const inputs = document.querySelectorAll('input, textarea, select');
inputs.forEach(input => {
    input.addEventListener('focus', () => input.style.boxShadow = '0 0 10px #f1c40f');
    input.addEventListener('blur', () => input.style.boxShadow = 'none');
});


/* ============================
   POSITIONS ZA SLIDER / KARUSEL
=============================== */
...
/* ============================
   KLASA ZA CIRCULAR SLIDER / KARUSEL
=============================== */
...
class CircularSlider {
    ...
    // Postavljanje početnih pozicija kartica
    applyPositions() {
    ...
    rotate(direction) {
        ...
        this.cards.forEach((card, index) => {
            let newIndex = direction === "next" ? (index - 1 + this.totalCards) % this.totalCards : (index + 1) % this.totalCards;
            const pos = positions[newIndex];

            gsap.set(card, { clipPath: pos.clip });
            gsap.to(card, { height: pos.height, duration: 0.5, ease: "power2.out" });
            gsap.to(card, { transform: `translateZ(${pos.z}px) rotateY(${pos.rotateY}deg) translateY(${pos.y}px)`, duration: 0.5, ease: "power2.out" });
        });
...
/* ============================
   ANIMACIJA AUTO-SCROLL KARUSELA
=============================== */

const sliderTrack = document.getElementById('sliderTrack');
let speed = 3; // brzina u pikselima po frame-u
let pos = 0;
...
function animateSlider() {
    ...
    if (Math.abs(pos) >= trackWidth - containerWidth) pos = 0;
...
/* ============================
   OTVARANJE FORME ZA RECEPT
=============================== */
document.getElementById("openRecipeForm").addEventListener("click", () => {
